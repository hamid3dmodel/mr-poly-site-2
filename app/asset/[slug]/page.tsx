...
async function readAssetsFile(): Promise<Asset[]> {
    const assetsPath = path.join(process.cwd(), "data", "assets.json");
    try {
        const file = await fs.readFile(assetsPath, "utf8");
        const clean = file.replace(/^\uFEFF/, "");
        const parsed = JSON.parse(clean);
        return Array.isArray(parsed.assets) ? parsed.assets : parsed.assets || [];
    } catch (err) {
        console.error("Failed to read assets.json:", err);
        return [];
    }
}
...
export default async function AssetPage({ params }: AssetPageProps) {
    ...
    const asset = assets.find((item) => item.slug === slug || item.id === slug);
                ...
                <div className="grid gap-12 lg:grid-cols-[1.2fr_0.8fr]">
                    <div>
                        <div className="overflow-hidden rounded-[var(--radius-xl)] border border-[var(--color-border)] bg-[var(--color-surface)]">
                            {asset.thumbnail ? (
                                <img src={asset.thumbnail} alt={asset.name} className="w-full object-cover" />
                            ) : (
                        ...
                        <div className="mt-6 grid grid-cols-3 gap-4">
                            {asset.gallery.map((image) => (
                                <div key={image} className="overflow-hidden rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface)]">
                                    <img src={image} alt={asset.name} className="aspect-video w-full object-cover" />
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <h1 className="text-4xl font-bold tracking-tight sm:text-5xl">{asset.name}</h1>

                        <p className="mt-6 leading-8 text-[var(--color-text-secondary)]">{asset.description}</p>
                        ...
                        <div className="mt-10">
                            ...
                            <div className="mt-4 overflow-hidden rounded-[var(--radius-md)] border border-[var(--color-border)]">
                                {Object.entries(asset.technicalSpecs || {}).map(([key, value]) => (
                                    <div key={key} className="flex justify-between gap-6 border-b border-[var(--color-border)] px-4 py-3 text-sm last:border-b-0">
                        ...
                        <div className="mt-10">
                            ...
                            <div className="mt-4 flex flex-wrap gap-3">
                                {asset.stores.map((store) => (
                                    <Button key={store.name} href={store.url} variant="secondary">
                                        {store.name}
                                    </Button>
                                ))}
                            </div>
